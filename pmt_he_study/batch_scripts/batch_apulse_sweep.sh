...
# [2] 14.04.20 : (W.Q) added the full path to all temporary directories that this
#     code generates. Removed the redundant commands
#
# USER INPUTS:
# ============
# Descriptive tag (will be used for naming of directory, scripts & log files):
# This will be the date which you start the batch
# tag="200422_apulse_sweep"
...
#
# File list (made by hand for the time being, not updated automatically)
#file_list=`cat $root_dir/batch_files/sweep_file_names_200422_200517.ascii`
ls $root_dir/data/raw_xml_files/*/*$voltage* /unix/nemo3/PMT_He_Study_nemo3/data/raw_xml_files/*/*$voltage* > $root_dir/batch_files/$script_log_dir/$file_filenames
data_file_list=`cat $root_dir/batch_files/$script_log_dir/$file_filenames`
#
# ROOT macro to use:
root_macro="$root_dir/supernemopmts/apulse_sweep.cpp"
#
#
# Batch queue to use:
queue="long"
...
do
  ...
  xml_file=${file##*/}
  base=${xml_file%.*}
  time=`echo $base | awk -F '_t' '{print $2}'`
  root_file="$root_dir/ROOT_files/Sweep_Files/$voltage$volt/$date$score$time$score$base.root"
  ...
  if test -f "$root_file"; then
    ...
    #
    interactive_command="root.exe -q -b $macro_plus_arguments >& $root_dir/batch_files/$script_log_dir/logs/batch_file_$ifile.log"
