...
void waveform_reader() {
	...
	resultsFile.open("../archive/22.2.19/short/channel_0/22_02_Ch0_waveforms.dat");
  
	ofstream currentOutFile;
  
	// Make an output file to store selected waveforms:
	TFile* histFile = new TFile("../ROOT_files/22.2.19/short/channel_0/selected_waveforms.root","RECREATE");
		...
		*/
		...
		while (getline(resultsFile, data) && waveform_counter < nWaveforms_Requested) {
			...
			if (counter == pointsCounter - 1) {
				...
				double Total_Area_Actual = abs(area);
                ...
                //3000
                //500
				if (Total_Area_Actual > 3000. && high_energy_counter < 10) {
					hist->Write();
					high_energy_counter++;
				} 
				if (Total_Area_Actual < 500. && low_energy_counter < 10){
					hist->Write();
					low_energy_counter++;
				}else {
					delete hist;	
